...
def main( c_host, e_host):
	...
	for registro in registros:
		#altera registro no Cassandra
		cassandra.execute("""
			UPDATE tweets SET user = %(user)s, content = %(content)s, retweet = %(retweet)s, date = %(date)s, likes = %(likes)s, last_update = %(last_update)s
			WHERE id =  %(id)s
		""",
		{'id' : registro.id, 'user' : registro.user + '_update', 'content' : registro.content + '_update', 'retweet' : random.randint(1, 10), 'date' : datetime.now(), 'likes' : random.randint(1, 10), 'last_update' : datetime.now()})
